...
export default function Payment() {
  ...
  if (!pet)
    return <div>No pet data available. Please go back and select a pet.</div>;
  ...
  const handleContinue = async () => {
    const { contact, delivery, city, firstName, address, paymentMethod } =
      formData;
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/purchase/buy-now",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        toast.success(
          "Your order was placed! Seller will contact you soon. Thank you for your purchase.",
          {
